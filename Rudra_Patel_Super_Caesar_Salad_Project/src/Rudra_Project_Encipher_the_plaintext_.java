...
public class Rudra_Project_Encipher_the_plaintext_ {
     ...
     * Returns: Main_Answer.
     */
    static String encipher_the_plaintext_(String secretCode, int[] shift_key,String Key_Word) {
        ...
        do {
            if (values_of_cipher_TextIs[k] < 65 || values_of_cipher_TextIs[k] > 90) {
                ...
                if ((values_of_cipher_TextIs[k] + shift_key[j]) > 90) {
                    decipher_TextIs_Chr[k] = (char) (64 + shift_key[j]);
                    k++;
                    j++;
                    if (j >= length) {
                        j = 0;
